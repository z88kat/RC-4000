...
serialPort.on("open", function () {
    console.log("-- Serial Connection Active --".green);
});
...
const writeToPort = (array) => {
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (!serialPort.isOpen) {
            console.error('Serial port is not open'.brightRed);
...
const sendData = async () => {
    ...
    return new Promise((resolve, reject) => {
        ...
        console.log('Sending data to watch'.brightGreen);

        let data = buildRCSerialData();
        if (!data) reject('No data to send');

        writeToPort(data).then((data) => {
            console.log('Data sent to watch. Please wait for transfer to complete.'.blue);
            resolve();
        }).catch((err) => {
