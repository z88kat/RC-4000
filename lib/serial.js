...
const sendData = async () => {
    ...
    return new Promise((resolve, reject) => {
        ...
        writeToPort(data).then((data) => {
            console.log(chalk.green('Data sent to watch. Please wait for transfer to complete.'));
            resolve();
        }).catch((err) => {
...
const buildRCSerialData = function (data) {
    ...
    if (array.length !== 2051) {
        console.log(chalk.red('Error: Data length is not 2051 bytes. It is ' + array.length + ' bytes'));
    }
