...
export default class WeeklyAlarm extends Label {
    ...
    // Set the data entry alarm label
    setDataLabel(index, label) {
    ...
    toggleAMPM(index) {
        ...
        item.timeString = item.hour + ':' + item.minute + ' ' + (item.ampm === 'A' ? 'AM' : 'PM');
        item.full_label = item.label + item.dayOfWeek + ' ' + item.dayString + ' ' + item.ampm + ' ' + item.hour + ':' + item.minute;
    }
    ...
    setHour(index, hours) {
        ...
        item.hour = hours;
        item.timeString = hours + ':' + item.minute + ' ' + (item.ampm === 'A' ? 'AM' : 'PM');
        item.full_label = item.label + item.dayOfWeek + ' ' + item.dayString + ' ' + item.ampm + ' ' + item.hour + ':' + item.minute;
    }
    ...
    setMinute(index, minutes = 0) {
        ...
        item.minute = minutes;
        item.timeString = item.hour + ':' + minutes + ' ' + (item.ampm === 'A' ? 'AM' : 'PM');
        item.full_label = item.label + item.dayOfWeek + ' ' + item.dayString + ' ' + item.ampm + ' ' + item.hour + ':' + item.minute;
    }

    setDayOfWeek(index, dayOfWeek) {
        let data = super.getData();
        let item = data[index];
        item.dayOfWeek = dayOfWeek + ''; // make sure its a string
    ...
    addRawData(label) {
        ...
        let d = {
            ...
            hour: hour,
            minute: minute,
            timeString: hour + ':' + minute + ' ' + (ampm === 'A' ? 'AM' : 'PM'),
            full_label: label + dayOfWeek + ' ' + dayString + ' ' + ampm + ' ' + hour + ':' + minute
        }
